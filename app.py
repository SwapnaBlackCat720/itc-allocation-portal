# app.py (v51 - FINAL with Enhanced AI Insights)
...
def get_allocation_recommendations(df, budget_multiplier, roas_weight):
    ...
    params = {
        ...
        "n_estimators": 100, "num_leaves": 20, "learning_rate": 0.1
    }
...
if not st.session_state.get("authentication_status", False):
    ...
    try:
        ...
        if all(col in original_df.columns for col in ['Date', 'Content Issue Flag']): df_copy = original_df.copy(); df_copy['Date'] = pd.to_datetime(df_copy['Date'], errors='coerce'); df_copy.dropna(subset=['Date'], inplace=True); end_date = df_copy['Date'].max(); start_date = end_date - timedelta(days=2); recent_issues = df_copy[(df_copy['Date'] >= start_date) & (df_copy['Content Issue Flag'].astype(str).str.lower() == 'yes')]; unresolved_issues = recent_issues[~recent_issues.index.isin(st.session_state.get('resolved_issues', set()))]
        ...
        if all(col in oos_df.columns for col in ['Time', 'Stock_Left']):
            ...
            unresolved_oos = recent_oos[~recent_oos.index.isin(st.session_state.get('resolved_oos', set()))]
        unresolved_oos_count = len(unresolved_oos)
        ...
        with tab1:
            if 'final_df' in st.session_state:
                ...
                # <<< --- RESTORED & ENHANCED INSIGHTS SECTION --- >>>
        ...
        with tab3:
            ...
            if unresolved_issues_count == 0: st.success("✅ All Clear!")
                ...
                for index, row in unresolved_issues.iterrows():
        ...
        with tab4:
            ...
            if unresolved_oos_count == 0:
                ...
                for index, row in unresolved_oos.iterrows():
                    with st.container():
