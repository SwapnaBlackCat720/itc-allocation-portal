# app.py (v56 - FINAL, DEFINITIVE FIX for Caching and UI)
...
# <<< --- DEFINITIVE CACHING FIX: Add underscores to the UI element arguments --- >>>
@st.cache_data
def get_allocation_recommendations(df, budget_multiplier, roas_weight, _progress_bar, _status_text):
    _status_text.text("Step 1/4: Preparing and cleaning data..."); time.sleep(0.5)
    ...
    _progress_bar.progress(20)

    _status_text.text("Step 2/4: Aggregating performance metrics..."); time.sleep(0.5)
    ...
    _status_text.text("Step 3/4: Training AI model..."); time.sleep(0.5)
    ...
    _status_text.text("Step 4/4: Calculating final budget allocation..."); time.sleep(0.5)
    ...
    _status_text.text("Calculation complete!")
    return df_agg
...
def login_form():
    # <<< RESTORED UI: Polished login form
    with st.container():
        ...
        with col2:
            ...
            with st.form("login_form"):
                username = st.text_input("Username"); password = st.text_input("Password", type="password"); st.markdown("<br>", unsafe_allow_html=True)
...
# <<< RESTORED UI: Custom CSS for professional look
st.markdown("""
<style>
    .main .block-container { padding-top: 2rem; padding-bottom: 2rem; }
    .st-emotion-cache-z5fcl4 { border: 1px solid rgba(44, 62, 80, 0.1); border-radius: 10px; box-shadow: 0 4px 12px 0 rgba(0,0,0,0.05); padding: 25px; background-color: white; }
    h1, h2, h3 { color: #2c3e50; }
    .st-expander { border: none !important; box-shadow: none !important; background-color: #f0f2f6; }
    .st-expander header { font-size: 18px; color: #1a73e8; }
...
if not st.session_state.get("authentication_status", False):
    ...
    try:
        ...
        if st.sidebar.button("🚀 Run Predictive Allocation", type="primary", use_container_width=True):
            # <<< DEFINITIVE CACHING FIX: Passing the UI elements to the cached function
        ...
        if all(col in oos_df.columns for col in ['Time', 'Stock_Left']):
            ...
            time_filter = datetime.now() - timedelta(minutes=30); recent_oos = df_oos_copy[(df_oos_copy['Simulated_Timestamp'] >= time_filter) & (df_oos_copy['Stock_Left'] <= 5)]; unresolved_oos_count = len(recent_oos[~recent_oos.index.isin(st.session_state.get('resolved_oos', set()))])
